...
- (IBAction)playOrPause:(UIButton *)sender {
    ...
    if (sender.selected) {
        ...
        [[RemotePlayer shareInstance] playWithURL:url isCache:NO];
    }else {
...
- (void)setVoiceModel:(DownLoadVoiceModel *)voiceModel
{
    ...
    // 动态的获取, 播放状态
    // url
    if ([url isEqual:[RemotePlayer shareInstance].url]) {
        // 判断状态
        RemoteAudioPalyerState state = [RemotePlayer shareInstance].state;
        if (state == RemoteAudioPlayerStatePlaying || state == RemoteAudioPlayerStateLoading) {
            self.palyOrPauseBtn.selected = YES;
        }else {
            self.palyOrPauseBtn.selected = NO;
        }
    }else {
        self.palyOrPauseBtn.selected = NO;
    }
...
{
    RemoteAudioPalyerState state = [notice.userInfo[@"playState"] integerValue];
    NSURL *url = notice.userInfo[@"playURL"];
